...
### Audio Resolution Chain (common → tts)
...
2. **TTS** — via the configured audio model. Single words also fall back to TTS when no common clip exists; multi-word phrases always go straight to TTS (sentences/listening stay TTS).

Both `POST /api/vocabulary/:id/audio` and `GET /api/vocabulary/common-audio` follow this chain.
